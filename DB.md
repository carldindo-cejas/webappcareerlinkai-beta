...
This file gathers the SQL used by the app database so you can paste it into a schema-diagram tool.

Source files:
- `apps/worker/schema.sql`
- `apps/worker/migrations/0001_profile_basics.sql`
- `apps/worker/migrations/0002_counselor_profile_and_venue.sql`
- `apps/worker/migrations/0003_notifications.sql`
- `apps/worker/migrations/0004_ai_chat.sql`

## Consolidated Current Schema

Use this section for schema-diagram generation. It reflects the current database structure after applying the base schema and migrations.
...
CREATE INDEX idx_departments_counselor ON departments(counselor_id);
CREATE INDEX idx_departments_join_code ON departments(join_code);
CREATE INDEX idx_department_members_student ON department_members(student_id);
CREATE INDEX idx_activity_counselor_created ON activity(counselor_id, created_at DESC);
CREATE INDEX idx_seminars_department ON seminars(department_id, scheduled_at DESC);
CREATE INDEX idx_seminar_invites_student ON seminar_invites(student_id, status, created_at DESC);
CREATE INDEX idx_notifications_user_created ON notifications(user_id, created_at DESC);
CREATE INDEX idx_ai_sessions_student ON ai_chat_sessions(student_id, updated_at DESC);
